...
Проект разворачивается в трех Docker контейнерах: backend-приложение API, postgresql-база данных и nginx-сервер. Четвертый контейнер с фронтендом используется только для сборки файлов и после запуска останавливается и далее уже не работает. 
